...
class FlexiblePredictor:
    ...
    def _fit_model(self, model, X, y):
        ...
        # 에포크 수 결정 (더 효율적인 학습)
        if data_size < 100:
            epochs = min(config.LSTM_EPOCHS_SMALL_DATA, 50)  # 최대 50 에포크
        else:
            epochs = min(config.LSTM_EPOCHS_LARGE_DATA, 100)  # 최대 100 에포크
        ...
        callbacks = [
            EarlyStopping(
                ...
                patience=min(config.EARLY_STOPPING_PATIENCE, 15),  # 더 빠른 종료
            ...
            ReduceLROnPlateau(
                ...
                patience=min(config.REDUCE_LR_PATIENCE, 8),  # 더 빠른 학습률 감소
    ...
    def _evaluate_and_log(self, model, X, y, scaler, history, nationality, purpose):
        ...
        if len(val_X) > 0:
            ...
            # 성능 기준 가져오기
            realistic_thresholds = self.get_improved_thresholds(len(X))
            
            # 성능 메트릭 계산
            metrics = self.calculate_comprehensive_metrics(
                np.array(y_true_final), np.array(y_pred_final), purpose, realistic_thresholds
            )
            
            # 학습 로그 캡처 (올바른 매개변수 전달)
            training_log = self.capture_training_logs(history, nationality, purpose, len(X))
    ...
    def predict_future_months(self, nationality, purpose, target_months):
        """미래 월별 예측 실행 - 개선된 시퀀스 생성 및 예측 로직"""
        ...
        if len(combo_data) == 0:
            print(f"데이터 없음: {nationality}-{purpose}")
            return None
        ...
        # 예측용 시퀀스 생성
        sequence_length = config.LSTM_SEQUENCE_LENGTH_LARGE_DATA if len(features) >= 200 else config.LSTM_SEQUENCE_LENGTH_SMALL_DATA
        
        # 최근 데이터로 예측 시퀀스 생성 (스케일링 적용)
        recent_data = features.tail(sequence_length)
        # Feature names를 유지하면서 스케일링
        scaled_sequence = scaler.transform(recent_data)
        
        predictions = []
        
        for i, target_month in enumerate(target_months):
            # 예측용 입력 데이터 준비 (올바른 형태)
            # scaled_sequence는 이미 numpy 배열이므로 .values 제거
            ...
            # 역스케일링
            pred_rescaled = self._inverse_scale_single(pred_scaled, scaler)
            
            # 목적별 역정규화 적용
            pred_rescaled = self._denormalize_single_value(pred_rescaled, purpose)
            
            # 예측 결과 저장
            predictions.append({
                "month": target_month,
                "value": max(0, int(pred_rescaled)),  # 음수 방지
                "type": "predicted"
            })
            
            # 다음 예측을 위해 시퀀스 업데이트 (개선된 방식)
            if i < len(target_months) - 1:
                # 새로운 예측값으로 시퀀스 업데이트
                new_row = recent_data.iloc[-1].copy()
                new_row.iloc[0] = pred_rescaled  # 첫 번째 컬럼이 타겟값
                
                # 시퀀스에서 가장 오래된 행 제거하고 새로운 행 추가
                recent_data = pd.concat([recent_data.iloc[1:], pd.DataFrame([new_row])], ignore_index=True)
                
                # 업데이트된 시퀀스를 다시 스케일링 (feature names 유지)
                scaled_sequence = scaler.transform(recent_data)
        
        print(f"{nationality}-{purpose} 예측 완료: {len(predictions)}개월")
        return predictions
    ...
    def _denormalize_single_value(self, value, purpose):
        ...
        try:
            if purpose == "관광":
                ...
                # 기타 목적은 그대로 반환하되 최소값 보장
                denormalized = max(1, value)
            
            return denormalized
        except Exception as e:
            print(f"역정규화 오류: {e}")
            # 기본값 반환
            return 100 if purpose == "관광" else 10
    ...
    def _create_visualization_layout(self, purpose_data):
        """시각화 레이아웃 생성"""
        num_purposes = purpose_data["num_purposes"]
    ...
    def _create_overview_graph(self, fig, gs, nationality, purpose_data, start_date, end_date):
        ...
        # 우측 Y축 생성
        ax_right = ax_overview.twinx() if purpose_data["right_purposes"] else None

        # 색상 설정
        colors = ["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFEAA7", "#DDA0DD", "#F4A460"]
        
        # 좌측 Y축 목적들 그리기
        self._plot_left_axis_purposes(ax_overview, purpose_data, colors)
        
        # 우측 Y축 목적들 그리기
        if ax_right:
            self._plot_right_axis_purposes(ax_right, purpose_data, colors)
        
        # 코로나 기간 및 예측 기간 표시
        self._add_period_highlights(ax_overview, purpose_data, start_date, end_date)
        
        # 축 설정 및 범례
        self._setup_overview_axes(ax_overview, ax_right, nationality)

    def _plot_left_axis_purposes(self, ax, purpose_data, colors):
        """좌측 Y축 목적들 그리기"""
        color_idx = 0

        for purpose in purpose_data["left_purposes"]:
            color = colors[color_idx % len(colors)]
            color_idx += 1

            combo_data = purpose_data["all_combo_data"][purpose]
            display_data = combo_data.tail(60) if len(combo_data) > 0 else pd.DataFrame()

            if len(display_data) > 0:
                # 주요 목적 vs 기타 목적 구분
                if purpose == purpose_data["max_purpose"]:
    ...
    def _plot_right_axis_purposes(self, ax_right, purpose_data, colors):
        """우측 Y축 목적들 그리기"""
        color_idx = len(purpose_data["left_purposes"])
        
        for purpose in purpose_data["right_purposes"]:
            color = colors[color_idx % len(colors)]
            color_idx += 1
    ...
    def _add_period_highlights(self, ax, purpose_data, start_date, end_date):
        """코로나 기간 및 예측 기간 하이라이트"""
        # 코로나 기간 마스크
        if purpose_data["num_purposes"] > 0 and len(purpose_data["all_combo_data"][purpose_data["purposes"][0]]) > 0:
            first_combo_data = purpose_data["all_combo_data"][purpose_data["purposes"][0]]
    ...
    def _setup_overview_axes(self, ax_overview, ax_right, nationality):
        """상단 그래프 축 설정 및 범례"""
    ...
    def _create_individual_graphs(self, fig, gs, nationality, purpose_data, start_date, end_date):
        """개별 목적별 그래프 생성"""
        for idx, purpose in enumerate(purpose_data["purposes"]):
    ...
    def _create_single_purpose_graph(self, ax, nationality, purpose, purpose_data, start_date, end_date):
        ...
        if len(combo_data) == 0:
            ax.text(0.5, 0.5, f"{purpose}: 데이터 없음", 
                   ha='center', va='center', transform=ax.transAxes, fontsize=16)
            ax.set_title(f"{nationality} - {purpose} (데이터 없음)", fontsize=16, fontweight="bold")
            return

            # 실제 데이터 플롯
        display_data = combo_data.tail(60)
        ax.plot(display_data["날짜"], display_data["입국자수"], 
               color="#FF6B6B", linewidth=3, label="실제 데이터", marker="o", markersize=4)

        # 예측 데이터 플롯 (있는 경우)
        if purpose in purpose_data["results"] and purpose_data["results"][purpose]:
            predictions = purpose_data["results"][purpose]
            pred_dates = [pd.to_datetime(p["month"] + "-01") for p in predictions if p["type"] == "predicted"]
            pred_values = [p["value"] for p in predictions if p["type"] == "predicted"]
            
            if pred_dates and pred_values:
                ax.plot(pred_dates, pred_values, 
                       color="#4ECDC4", linewidth=3, label="예측 데이터", 
                       linestyle="--", marker="s", markersize=6)

        # 코로나 기간 표시
        covid_data = combo_data[combo_data["코로나기간"] == 1]
        if len(covid_data) > 0:
            covid_start = covid_data["날짜"].min()
            covid_end = covid_data["날짜"].max()
            ax.axvspan(covid_start, covid_end, alpha=0.3, color="red", label="코로나 기간")

        # 예측 기간 표시
        pred_start = pd.to_datetime(start_date + "-01")
        pred_end = pd.to_datetime(end_date + "-01")
        ax.axvspan(pred_start, pred_end, alpha=0.2, color="orange", label="예측 구간")
    ...
    def _create_visualization_csv_report(self, nationality, purpose_data, start_date, end_date, timestamp):
        ...
        csv_path = f"{self.results_dir}/{nationality}_예측리포트_{timestamp}.csv"
        csv_df.to_csv(csv_path, index=False, encoding="utf-8-sig")
        print(f"예측 리포트 CSV 저장: {csv_path}")
