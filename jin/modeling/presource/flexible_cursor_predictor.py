...
class FlexiblePredictor:
    ...
    def load_data(self):
        ...
        self.data["날짜"] = pd.to_datetime(
            self.data["연도"].astype(str) + "-" + self.data["월"].astype(str)
        )
        self.data = self.data.sort_values("날짜")
        # 2. 코로나 플래그 feature 추가
        self.data["코로나기간"] = (
            (self.data["날짜"] >= config.COVID_START_DATE)
            & (self.data["날짜"] <= config.COVID_END_DATE)
        ).astype(int)
    ...
    def create_advanced_features(self, data):
        ...
        # 3. 전년동월대비증감률 feature 생성 및 처리
        features_data["전년동월대비증감률"] = (
            features_data["입국자수"] - features_data["입국자수"].shift(12)
        ) / (features_data["입국자수"].shift(12) + 1e-6)
        features_data["전년동월대비증감률"] = features_data["전년동월대비증감률"].replace(
            [np.inf, -np.inf], 0
        )
        features_data["전년동월대비증감률"] = features_data["전년동월대비증감률"].fillna(0)
        features_data["전년동월대비증감률"] = np.clip(features_data["전년동월대비증감률"], -1, 1)
        # 4. 모든 수치형 컬럼의 inf/NaN 처리 및 클리핑
        for col in features_data.select_dtypes(include=[np.number]).columns:
            features_data[col] = features_data[col].replace([np.inf, -np.inf], 0)
            features_data[col] = features_data[col].fillna(0)
            # ±100% 등으로 클리핑 (예: 증감률 등)
            if "증감률" in col:
                features_data[col] = np.clip(features_data[col], -1, 1)
    ...
    def train_purpose_model(self, nationality, purpose):
        ...
        # 목적별로 log1p 정규화 적용
        purpose_data = combo_data.copy()
        purpose_data["입국자수"] = np.log1p(purpose_data["입국자수"])
        # 이후 MinMaxScaler 등 추가 적용 가능
        ...
        sequence_length = (
            config.LSTM_SEQUENCE_LENGTH_SMALL_DATA
            if data_size < 100
            else config.LSTM_SEQUENCE_LENGTH_LARGE_DATA
        )
        ...
        callbacks = [
            ...
            EarlyStopping(
                monitor=(
                    config.EARLY_STOPPING_MONITOR if len(X_val) > 0 else "loss"
                ),  # 검증 데이터가 있으면 val_loss, 없으면 loss 모니터링
    ...
    def _predict_tourism_optimized(self, nationality, purpose, target_months):
        ...
        for idx, target_month in enumerate(target_months):
            ...
            if len(existing_data) > 0:
                ...
                print(
                    f"  🏖️ 관광 계절성 적용: {target_month} - {seasonal_factor:.2f} 곱적용 -> {pred_value:,.0f}"
                )
                ...
                if idx > 0:
                    ...
                    if pred_value < min_value:
                        ...
                        print(
                            f"  🛡️ 관광 최소값 보장: {target_month} - {min_value:,.0f}명 이상 유지"
                        )
    ...
    def _create_tourism_features(self, data):
        ...
        print(
            f"🏖️ 관광 특화 강화 특성 생성 완료: {len([col for col in features.columns if any(keyword in col for keyword in ['계절', '성수기', '휴가', '관광', '벚꽃', '단풍', '스키', '해수욕'])])}개 계절성 특성"
        )
...
def main(
    ...
    if auto_run:
        ...
        if end_date is None:
            end_date = "2026-12"
        print(
            f"자동 실행 모드: {nationality}, {purpose if purpose else '모든 목적'}, {start_date} ~ {end_date} 예측을 시작합니다."
        )
    else:
        # 코로나 데이터 처리 전략 선택
    ...
    if purpose is None:
        available_purposes = (
            predictor.data[predictor.data["국적"] == nationality]["목적"].unique().tolist()
        )
